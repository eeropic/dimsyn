class OscillatorProcessor extends AudioWorkletProcessor {
    ...
    process(inputs, outputs, parameters) {
        ...
        for (let i = 0; i < output[0].length; i++) {
            ...
            var midpoint = midpoints.length > 1 ? midpoints[i] : midpoints[0]
            var curvature = curvatures.length > 1 ? curvatures[i] : curvatures[0]
            ...
            */				//out_bp = 3.0f * (b3 - b4);
            // (1-2*frequencyN)
            const bpOutput = 3.0 * (this.b3 - this.b4)
            // + this.clamp(noise/2,0,1) * inp
